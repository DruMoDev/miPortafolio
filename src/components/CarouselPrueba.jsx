...
const CarouselPrueba = () => {
  ...
  const cardWidth = 100; // Ancho de cada tarjeta en el carrusel
  ...
  useEffect(() => {
    ...
    const scroll = () => {
      if (container) {
        ...
        if (scrollLeft >= container.scrollWidth - container.clientWidth) {
          ...
          container.scrollTo({
            left: scrollLeft,
            behavior: "auto", // Comportamiento automático para un desplazamiento suave
          });
        } else {
    ...
    const intervalId = setInterval(scroll, 20); // Velocidad de desplazamiento
